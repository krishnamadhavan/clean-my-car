...
**Implementation status (codebase):** Ops Modules **1–5** (auth, users, location, waitlist, vehicle catalog) are implemented under `/api/v1/ops/*`. Pricing (6) is still seed/SQL until ops pricing routes land. Consumer error e.g. `pricing_not_found` means Module **6** master data is missing for that city.
